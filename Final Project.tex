...
\begin{thm}
\textsc{Ranking} has an expected competitiveness of $1-1/e\approx0.63$. 
\end{thm}
...
\end{tikzpicture}
\caption{The matching $m_{\sigma}$ is shown in the graph above. Let $t = \sigma(v)$. Then the highlighted nodes represent the set $R_t$.}
\label{fig:Rt}
\end{figure}
\end{center}
...
our expected competitiveness equal to $S_{n}/n$). We can
...
\]
It turns out that $S_{t}$, and by extension also $S_{n}/n$,
...
\]
As a result, our expected competitiveness $S_{n}/n$ is lower
bounded by 
\[
...
\]
As $n$ approaches infinity, this value converges to $1-1/e$,
thus concluding our proof.

Now that we demonstrated that the expected competitiveness of \textsc{Ranking} is at least $1-1/e$, the question becomes,
...
\end{tikzpicture}
\end{center}
\caption{A graph demonstrating the tightness of the 1/2 competitive ratio for \textsc{Weighted-Greedy}.}
\end{figure}

Say that vertices arrive in the order $(u_1, u_2)$, and let $w_{v_1} = 1 + \epsilon$ for some $\epsilon > 0$ and $w_{v_2} = 1$. Then the greedy algorithm will match only $(u_1, v_1)$, and the optimal solution will match $(u_1, v_2), (u_2, v_1)$. As $\epsilon$ approaches 0, the competitive ratio approaches $1/2$.
...
\begin{thm}
\textsc{Perturbed-Greedy} achieves an approximation ratio of $1 - 1/e$ for the vertex-weighted online bipartite matching problem.
\end{thm}
...
such that no vertex has more than a total of one full matching (i.e., $\sum_{u \in U}{x_{uv}} \leq 1$ for all $v \in V$ and $\sum_{v \in V} x_{uv} \leq 1$ for all $u \in U$). By giving the optimal algorithm more power, we are able to upper bound the best matching of any algorithm.
...
\begin{defn}Let $b_v$ be a multiple of $p$ of a geometric random variable with parameter $p$. It represents the bin size for vertex $v$. Formally, we have
\[Pr[b_v = pt] = p(1-p)^{t-1}.\]
\end{defn}
...
& \sum_v{ \left ((1-p)^{-L^*_v/p} + \sum_{y<L^*_v}{f_v(y)(1 + L^*_v - x)(1-p)^{-y/p}} \right )} \leq n & \quad \forall{x}
\end{alignat}
\label{defn:lp}
\end{defn}
...
\begin{lem} For the variables as defined above, we have
\[f_v(x) = (1-p)^{(x/p)}q_v(x).\]
\begin{proof}
...
&$=$& $Pr[b_v > x]Pr[L^{\infty}_u(b_v) = x]$ \\ &&\\
&$=$& $(1-p)^{(x/p)}q_v(x)$
\end{tabular}
\end{center}

This follows from representing the probability of a resulting load being unsuccessful as the intersection of when the load is less than its threshold and the algorithm terminates with $x$. By independence, this separates giving the resulting expression where $(1-p)^{(x/p)}$ indicates that the matching fails $x/p$ times and $q_v(x)$ is the probability of the resulting load. We can represent the probability that load $x$ succeeds for vertex $v$ as $g_v(x)$ and find a similar expression. 
\end{proof}
\end{lem}
...
&$=$& $Pr[b_v = x]Pr[L^{\infty}_v(b_v) \geq x]$ \\ &&\\
&$=$& $p(1-p)^{(x/p-1)}\sum_{y \geq x}{q_v(y)}$
\end{tabular}
\end{center}

This follows from noting that a load is successful if it is equal to its threshold. But also, the resulting load of the algorithm if there is no limit on the load must give some load greater than $x$ or $x$ is not possibly obtained. This probability of the first term is $ p(1-p)^{(x/p-1)}$ while the probability of the second is the sum of each of these disjoint $q_v(y)$ events.
\end{proof}
\end{lem}

\begin{lem} Constraint 4.1 is correct. Formally, we have
...
\begin{lem} Constraint 4.2 is correct. Formally, we have
\[\sum_{y \leq x}{\sum_v{(1+L^*_u)f_u(y)}} + (1-p)^{x/p}\sum_{y>x}{(1-p)^{-y/p}(\sum_u{f_y(y)})} \leq n.\]
\begin{proof}
...
By taking the dual of this linear program given in Definition~\ref{defn:lp}, finding a feasible solution, and applying weak duality, \cite{key-4} was able to to obtain an upper bound on the number of failed matchings which \textsc{Stochastic-Balance} will encounter. The derivation of the competitive ratio is not given here, but we encourage interested readers to consult \cite{key-4} for the full proof.
