...
In the classic version of the problem, first introduced in \cite{key-1},
...
As expected, adaptive algorithms have more power than non-adaptive algorithms and are able to achieve better competitive ratios. An extension of \textsc{Ranking} seen earlier performs well when $p \leftarrow 1$ giving the same competitive ratio as the non-stochastic classic problem. But interestingly enough, a deterministic algorithm called \textsc{Stochastic Balance} performs better when $p \leftarrow 0$. And this algorithm's competitive ratio is $1/2$ when $p = 1$ showing that it is consistent with the theorem that any deterministic algorithm has competitive ratio of at most $1/2$ in the classic problem.
...
Like in \textsc{Ranking} analysis, we will prove the first constraint to show the flavor for how these are constructed and along the way prove the third constraint. The original paper itself only sketches out the proof of the second constraint.

\begin{proof} Let $L^{\infty}_u(\theta)$ be the load on vertex $u$ given that $\theta$ is modified such that $\theta_u = \infty$ or in words the load on $u$ when the algorithm finishes under the other vertices thresholds. 
...
\bibitem[{[}BM{]}]{key-2} B. Birnbaum and C. Mathieu, \textquotedblleft{}On-line
...
pp. 728-737. (http://www.cs.duke.edu/\textasciitilde{}debmalya/papers/focs12-matching.pdf)\end{thebibliography}
