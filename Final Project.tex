...
The algorithm is tasked with irreversibly matching the incoming
vertex if possible. Variants of this problem include considering weighted vertices and adding uncertainty if a matching will succeed.
These variants generalize the original problem to a wide variety of
applications particularly in the domain of online advertising.
...
and $V$ are two independent sets of vertices and $E$ is
...
vertex $u\in U$ is received, the edges incident on $u$ are revealed and the algorithm then either permanently
matches $u$ to an unmatched, adjacent vertex $v\in V$  or chooses to leave $u$ permanently unmatched.
The goal of the problem is to maximize the size of the matching.
For the purposes of this paper, we assume that $|U| = |V| = n$ and that $G$ has a perfect matching. Thus the size of the optimal matching is $n$.


\subsubsection{Useful Definitions}
\begin{defn}
Let $N(u)\subset V$ be the set of available vertices that $u$
can still be matched with. More formally, we define $N(u)$ as the set of vertices
in $V$ that are adjacent to a vertex $u\in U$, but still haven't
been matched by the algorithm.
\end{defn}
Note that $N(u)$ for a vertex $u\in U$ is revealed to the algorithm only once the
algorithm receives vertex $u$.
\begin{defn}
We define $m^*$ as an optimal matching, which we assume is a perfect matching. Furthermore we define $m^*(u)$ as the vertex $v \in V$ that $u$ is matched with.
\end{defn}
...
is optimal for deterministic algorithms. The algorithm works as follows. Upon reception of a vertex $u\in U$, \textsc{Greedy} matches $u$ to an arbitrary
vertex $v\in N(u)$ if $N(u)\ne\emptyset$, and otherwise leaves it unmatched.

\begin{thm}  
\textsc{Greedy} is a (1/2)-competitive algorithm for the classic online bipartite matching problem.
\end{thm}

\begin{proof}
It is easy to see that \textsc{Greedy} has a competitive ratio
...
vertex $u\in U$, at least one of $u$ or $m^{*}(u)$ must be in \textsc{Greedy}'s matching. Thus, our algorithm matches a minimum
...
The Ranking algorithm, which we will call \textsc{Ranking}, begins with an initialization phase that consists
...
can relate $m_{\sigma}$ with the optimal matching $m^{*}$ using
the following lemma.
\begin{lem}
...
when $u$ was received by the Ranking algorithm, $v$ was still unmatched.
...
$\sigma(v)=t$ is matched in $m_{\sigma}$, then $1-x_{t}\le\frac{1}{n}\sum_{1\le s\le t}x_{s}$
...
$V$ of rank that is less than $t$,. In other words for all $u\in U$
...
bounded by the probability that $u\in R_{t}$. Clearly, by the definition
of $x_{t}$, the probability that $v$ is not matched is $1-x_{t}$
and the expected size of $R_{t}=\sum_{1\le s<t}x_{s}$. If $u$ and
$R_{t}$ were independent, then the probability that $u\in R_{t}$
would simply be $\frac{|R_{t}|}{n}=\frac{1}{n}\sum_{1\le s<t}x_{s}$,
which would complete our lemma with $1-x_{t}\le\frac{1}{n}\sum_{1\le s<t}x_{s}\le\frac{1}{n}\sum_{1\le s\le t}x_{s}$
.
...
up. The correct, but less intuitive, proof demonstrates how choosing
vertices $v$ and $u$ randomly and independently of $\sigma$ can
result in having the relation that if vertex $v$ is not matched in
$m_{\sigma}$, then $u\in R_{t+1}$. In this case $u$ and $R_{t+1}$
are independent, which correctly proves the lemma. Again, we encourage
interested readers to read the correct proof in \cite{key-2}.
\end{proof}
...
is smallest when all of the inequalities are equalities, which gives
...
First we show that \textsc{Weighted-Greedy} has a competitive ratio of at least 1/2. Let $\sigma$ be an ordering of the nodes defined by decreasing order of weight. Let $u$ be an arbitrary vertex in $U$, and let $v = m^*(u)$ be a vertex not matched in $m_{\sigma}$. Then by Lemma 5, in $m^*$ $u$ must be matched to a vertex of weight at most $w_v$. But we can see that the optimal value `lost' by $u$ is at most equal to the weight of its match in $m_{\sigma}$. Therefore the total amount which $m_{\sigma}$ `loses' is $|m_{\sigma}|$. Hence, $|m_{\sigma}| \geq |m^*|/2$.
...
As in the unweighted case, no deterministic algorithm can do better than $\textsc{Weighted-Greedy}$. [[[ no proof given in paper? ]]
...
We want to generalize the ranking algorithm in order to account for the weighted case. We will present a new algorithm which we will call \textsc{Perturbed-Greedy}, and then we will show that this algorithm is equivalent to \textsc{Ranking} in the case where all weights are equal. This algorithm will begin by `perturbing' each of the weights of the vertices in $V$. Then it will follow the \textsc{Greedy} scheme as outlined above.
...
We will offer some intuition as to why the \textsc{Perturbed-Greedy} algorithm is an effective solution to the weighted vertex problem. For the classic online bipartite matching problem, we presented two algorithms: \textsc{Greedy} and \textsc{Ranking}. For very skewed edge weights, \textsc{Greedy} will perform quite well, because the loss from the optimal solution that each match has to charge will be much less than the value of the match. On the other hand, we have already noted that \textsc{Ranking} performs well on graphs where the range of weights is small.

To develop a good algorithm for the weighted case, we need something which combines the benefits of each of these algorithms. When weights are highly variable, \textsc{Perturbed-Greedy} is likely to use a ranking very similar to the ranking in $\textsc{Greedy}$, and therefore is able to find a good solution in that case. When weights are very similar, \textsc{Perturbed-Greedy} uses a ranking which is closer to a completely random ranking, and thus will behave well in this situation as well.

While we will not offer a full proof of the theorem here, we note that this is consistent with the competitive ratio of \textsc{Ranking}. It turns out that, as is the case with \textsc{Ranking}, this is the optimal competitive ratio for this problem. This result seems reasonable because we know that this is the optimal competitive ratio for the more specific version of this problem.

The proof of the approximation ratio of \textsc{Perturbed-Greedy} is quite similar to the proof of the approximation ratio of \textsc{Ranking} in Section ~\ref{section:ranking analysis}. We encourage readers to read this proof, as well as the proof of the optimality of the approximation ratio, in \cite{key-3}.
...
\subsection{Ranking Algorithm}\textsc{Ranking} is the same as previously presented with the ability to adaptively rematched failed matchings. \textsc{Ranking} randomly permutes the set of U vertices. Upon an incoming vertex to be matched, the algorithm matches it with the highest ranked neighbor that has not been successfully matched. The analysis introduces the notion of a good matching and a bad matching. A bad matching occurs when matched edge $(u, v)$ has $\sigma(u) < \sigma(m^*(v))$ . The intuition is that the algorithm matched a vertex too early (earlier than the optimal). Otherwise, the matching is a good match. $g(s)$ and $b(s)$ are respectfully defined to be the expected load of the vertex $u$ with $s = \sigma(u)$ due to good matches and bad matches. The linear program takes the following form where the adversary chooses $g(s)$ and $b(s)$ to minimize the expected number of matches under some algorithmic constraints. 
...
\begin{alignat}{3}
\text{minimize}  \quad & \sum_{s \leq n} m(s) & \nonumber \\
\text{subject to} \quad & m(s) = g(s) + b(s) & \quad \forall{s} \\
& \frac{1 - (1-p)^{\frac{1}{p}}}{p} - m(t) \leq \sum_{s \leq t}{\frac{b(s)}{n-s}} & \quad \forall{t} \\
&  \sum_{s \leq t}{\frac{s\cdot b(s)}{n-s}} \leq \sum_{s \leq t + 1}{g(s)} & \quad \forall{t} \\
& \sum_{t \leq n}{m(t)} \geq \frac{1}{2} + \frac{1}{2} \cdot \sum_{t \leq n}{g(t)} &
\end{alignat}
